...
double CalcTempGuelph(int guelphCounter, Weather guelph [])
{
    ...
    for(k = 0; k<=guelphCounter; k++)
    {
       
        //acculmates the elements in the array that contain the -400 temp
        if(guelph[k].temp == -400 )
        {
    ...
    //takes the total amount elements in array and subtracts the ones containing the -400 temp
    guelphCounter = k - error;
...
double CalcPercipGuelph(int guelphCounter,  Weather guelph []) 
{
    ...
    { 
        ...
        //checks if perciption is less then -1 and excludes it from the total 
        if(guelph[k].precip <= -1 )
        {
...
int main(int argc, char*argv[]) 
{
    ...
    //if the size of the guelph array holds data then call the function to calculate averages and perciption  
    if(guelphCounter!=-1)
    {
        //calls function to calculate the total average temp(sum) for the guelph array 
    ...
    //if the size of the sask array holds data then call the function to calculate averages and perciption  
    if(saskCounter!= -1)
    { 
       //calls function to calculate the total average temp(sum) for the sask array 
